...
\newcommand{\masterofritualdef}{%
	The model gains +1 Discipline, \textbf{Daemonic Summon}, and its Commanding Presence range is \textbf{set} to \distance{18}.%
}

% Daemon Symbiote
\newcommand{\singlemanifestation}{A single non-Guiding Manifestation}
...
\newcommand{\summonedcategorynote}{%
	The unit cannot be deployed normally, but only through \newrule{Daemonic Summon}.%
}
...
\newcommand{\theendisnigh}{The End is Nigh!}
\newcommand{\theendisnighdef}{%
	The model \textbf{must} select \spellformat{\thaumaturgyspellfive}{\thaumaturgy} and \spellformat{\occultismspellone}{\occultism} during Spell Selection. This overrides the Spell Selection rules connected to being a Wizard Adept. If the model performed any kind of move during a Player Turn, it cannot cast any spells in that Player Turn's Magic Phase. The range for spells cast by this model is doubled.%
}
...
\newcommand{\apostateautomatondef}{%
	The model's Armour is \textbf{set} to 3, and it gains +1 Resilience and \textbf{Crush Attack}.%
}
...
\textbf{\engineofdamnation{}} is	\zerotoXunitsperarmy{1} if the army includes any Gigantic Characters.%
}
