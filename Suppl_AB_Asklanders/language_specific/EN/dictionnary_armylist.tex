...
\newcommand{\deedsnotwordsdef}{%
	The model part gains \textbf{\battlefocus} and \textbf{\hatred} when in a unit that has \rnf{} models with \battlefever{}.%
}
...
\newcommand{\longshipraiddef}{%
	The model gains \textbf{\ambush{}}. During step 8 of the Pre-Game Sequence, nominate a unit of \AA{}sklanders, Huskarls, or Berserkers that includes no more than 30 models if \AA{}sklanders and no more than 20 models if Huskarls or Berserkers. This unit gains \textbf{\ambush{}}, and the model with Longship Raid must either be deployed or Ambush in this unit. Units using Longship Raid do not roll for Ambush but automatically enter the Battlefield in the second Player Turn. All units using Longship Raid must arrive from the same Board Edge. Otherwise follow the rules for Ambush.%
}

\newcommand{\berserkersbearpeltchiefoption}{Replace Heavy Armour with Shield and Berserker's Bear Pelt}
...
% Huskarls
\newcommand{\replaceshieldwithgw}{Replace \shield{} with \gw{}}
...
\newcommand{\icetrolldef}{%
	The model's Armour is \textbf{set} to 2. Flaming Attacks must reroll successful to-wound rolls against the model.%
}
\newcommand{\stupid}{Stupid}
\newcommand{\stupiddef}{%
	At the start of each friendly Player Turn, each unengaged non-Fleeing unit with one or more models with Stupid must take a Discipline Test. If the test is failed, all models in the unit become Shaken until the end of the Player Turn, and in the Movement Phase, directly after Rallying Fleeing units, the unit must move \distance{D6} directly forwards, stopping \distance{1} before Impassable Terrain or other units.%
}
...
\newcommand{\monstrousfamiliardef}{%
	The model gains \textbf{\wizardapprentice{}}. Instead of selecting spells as normal, it must select one of the following spells (during Spell Selection): \spellformat{\thaumaturgyspellfour}{\thaumaturgy}, \spellformat{\witchcraftspellone}{\witchcraft}, or \spellformat{\frostbite}{\hereditaryspell}.%
}
