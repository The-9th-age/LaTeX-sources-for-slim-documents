...
When an enemy unit declares a Charge against a unit with \martialdiscipline{}, a single friendly unit \newrule{containing one or more model} with \covervolley{} may immediately perform a Stand and Shoot Charge Reaction with the following conditions and restrictions:%
\begin{smallitemize}%
...
\item The unit \textbf{must} use Stand and Shoot before the Charged unit declares its Charge Reaction. \newrule{If the Charge is no longer possible after the unit's Stand and Shoot (e.g. due to the Charging unit being destroyed or failing a Panic Test), the Charged unit does not declare any Charge Reaction.}%
