...
\newcommand{\lifeseedfeathersdef}{%
	Attacks made with this weapon become \textbf{\magicalattacks{}}. Instead of firing this \sylvanlongbow{} using the standard rules for Sylvan Longbows and Longbows, the wielder may apply the following rules:\newline
	\range{30}, \shots{\textbf{always} 1}, Strength and Armour Penetration vary depending on the wielder's distance to the target:
	\begin{smallitemizeitem}%
		\item \St{} 4, \AP{} 1 if within \distance{10}
		\item \St{} 5, \AP{} 2 if more than \distance{10} and within \distance{20}
		\item \St{} 6, \AP{} 3 if more than \distance{20} and within \distance{30}, and the attack gains \textbf{\multiplewounds{2}{}}
...
\newcommand{\mistwalkersmirrordef}{%
	One use only. If the bearer's unit consists entirely of Standard Height Infantry models, is unengaged, and is fully inside a Forest Terrain Feature that doesn't contain any enemy models, the unit may teleport to any other Forest Terrain Feature on the Battlefield. This special movement is resolved at the end of the owner's Movement Phase. When teleporting, the unit \textbf{must} be placed fully inside the target Forest. It may appear in any legal formation but \textbf{must} follow the Unit Spacing rule. The unit counts as having performed a March Move.%
}
