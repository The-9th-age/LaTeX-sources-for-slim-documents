...
\newcommand{\scourgeofkingsdef}{%
	When using this weapon, the wielder's Attack Value is \textbf{set} to 6. Attacks made with this weapon become \textbf{\magicalattacks} and must reroll failed to-wound rolls when fighting a Duel.%
}
...
\newcommand{\banneroftheentombeddef}{%
	If taken by a Character, the bearer gains \textbf{\undergroundambush{}}. If taken by a \rnf{} model, the bearer's unit gains \undergroundambush{} and additional models cannot be added to the unit during Army List creation. Standard Height models using this banner to Ambush must arrive in a formation containing exactly 5 models per rank (except for the last) and cannot make a Reform (or a Swift Reform) during this Player Turn.\vspace*{5pt}\newline
...
\newcommand{\sandstormcloakrestriction}{Models on foot only}
