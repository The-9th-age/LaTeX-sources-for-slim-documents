...
\newcommand{\godslayerdef}{%
	The wielder gains +1 Attack Value when using this weapon. Attacks made with this weapon become \textbf{\divineattacks} and \textbf{\magicalattacks}, and gain \textbf{\multiplewounds{2}{\against{} \aegis{}}} (note that the latter also applies against models with Aegis Saves with Conditional Application).%
}

\newcommand{\scourgeofkingsdef}{%
	When using this weapon, the wielder's Attack Value is \textbf{set} to 6. Attacks made with this weapon become \textbf{\magicalattacks} and \textbf{must} reroll failed to-wound rolls when fighting a Duel.%
}
...
\newcommand{\banneroftheentombeddef}{%
	If taken by a Character, the bearer gains \textbf{\undergroundambush{}}. If taken by a \rnf{} model, the bearer's unit gains \undergroundambush{} and additional models cannot be added to the unit during Army List creation. Standard Height models using this banner to Ambush \textbf{must} arrive in a formation containing exactly 5 models per rank (except for the last) and cannot make a Reform (or a Swift Reform) during this Player Turn.\vspace*{5pt}\newline
	\bannerentombedpic{A unit with \undergroundambush{} also counts towards \entombed{}.}%
}

\newcommand{\crownofthepharaohsrestriction}{Pharaohs and Nomarchs only}
\newcommand{\crownofthepharaohsdef}{%
	The bearer increases the range of its \commandingpresence{} by \distance{6}. At the start of each of your Player Turns, the bearer may lose \undyingwill{} until the start of your next Player Turn and choose a friendly unit within \distance{12}. This unit gains \undyingwill{} until the start of your next Player Turn.%
}

\newcommand{\sacredhourglassdef}{%
	The bearer may reroll failed Casting Attempts that were rolled using 2 Magic Dice (by rerolling both Magic Dice).\columnbreak%
}
...
\newcommand{\bookofthedeaddef}{%
	...
	The spell's range is changed to \distance{12} Aura.%
}
...
\newcommand{\ankhofnapteshdef}{%
	The bearer gains \textbf{\hierophant}. \rnf{} models in the bearer's unit gain \textbf{\fortitude{} (6+)}.%
}

\newcommand{\sandstormcloakdef}{%
	The bearer gains \textbf{\fly{5}{15}}, \lighttroops{}, and \swiftstride{}, and can perform a \textbf{\sweepingattack{}} that causes 2D6 hits with Strength 2 and Armour Penetration 1.%
}
