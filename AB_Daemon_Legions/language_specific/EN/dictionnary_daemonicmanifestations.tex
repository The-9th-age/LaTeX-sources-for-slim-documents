...
A side with one or more \newrule{models with} Unnatural Roots Engaged in Combat at the end of the Round of Combat adds +1 to its Combat Score.\newpage% remove \newpage when translating
}
...
Each Close Combat Attack allocated towards the bearer \guidingversion{and \rnf{} models in its unit} that rolls a natural \result{1} on its to-hit roll \newrule{is distributed onto} the attacking model's Health Pool.%
}
...
Attacks made against the model \guidingversion{and against \rnf{} models in its unit} no longer are \divineattacks{} (if they were).%
}
...
At the end of each Round of Combat during which the model's unit was Engaged in Combat, and during which the model \guidingversion{and \rnf{} models in the bearer's unit} caused at least three Health Point losses with Close Combat Attacks, the unit Raises D3 Health Points.%
}
...
At the end of each Round of Combat during which the model's unit was Engaged in Combat, and the model \guidingversion{and \rnf{} model in the bearer's unit} caused at least three Health Point losses by Close Combat Attacks, you gain D3 Veil Token to your Veil Token pool.%
}
\newcommand{\diviningsnoutdef}{%
When Charging a unit that contains more than one Special Item, the model \guidingversion{and each \rnf{} model in its unit} gains +\distance{2} Advance Rate for Charge Range rolls \newrule{and must reroll failed Charge Range rolls}. The effects only apply \newrule{in the Charge Phase and only} if all models in the unit are affected by \diviningsnout{}.%
}
...
The model's Close Combat Attacks \guidingversion{and those of \rnf{} models in its unit} gain +1 Strength and +1 Armour Penetration, but each of its Close Combat Attacks \guidingversion{and those of \rnf{} models in its unit} with a natural to-hit roll of \result{1} is distributed onto the \newrule{attacking model's Health Pool}.%
}
