...
\newcommand{\dexteroustentacles}{Dextrous Tentacles}% deprecated
...
\newcommand{\tarskin}{Mirrored Scales}% deprecated
...
\newrule{A side with one or more units with one or more instances of Unnatural Roots Engaged in Combat at the end of the Round of Combat adds +1 to its Combat Score.}%
}
\newcommand{\sorcerousantennaedef}{%
\itemrestriction{0--2 per Army.}
\newrule{At the start of} Siphon the Veil in each of your Magic Phases, \newrule{choose a single model part in} each unit with one or more instances of this Manifestation. \newrule{The chosen model part gains \textbf{\channel{} (1)} until the end of the Magic Phase.}%
}
\newcommand{\markoftheeternalchampiondef}{%
If the \newrule{bearer} is not a Wizard, it becomes a \textbf{\wizardapprentice} that does not select spells as normal but always knows \spellformat{\spearofinfinity}{\hereditaryspell}. If the \newrule{bearer} is already a Wizard, it knows \spellformat{\spearofinfinity}{} in addition to its other spells and cannot select it during Spell Selection.%
}
\newcommand{\chargedtendrilsdef}{%
At the end of Siphon the Veil, each unit with one or more instances of this Manifestation allows the owner to store one additional Veil Token, up to a maximum of 6.%
}
\newcommand{\mirroredscalesdef}{%
\itemrestriction{\guidingversion{Dominant}.}
\newrule{Each Close Combat Attack allocated towards the bearer \guidingversion{and \rnf{} models in its unit} that rolls a natural \result{1} on its to-hit roll hits the attacking model's Health Pool instead.}%
}
\newcommand{\darkhidedef}{%
The model gains \textbf{\scout} with the following exception: it must be deployed fully inside the owner's Deployment Zone, and the owner must have deployed at least one unit normally.%
}
\newcommand{\hammerhanddef}{%
The model gains +1 Attack Value.%
}
\newcommand{\ironhuskdef}{%
The model\newrule{'s} Resilience \newrule{is \textbf{set} to 6}.%
}
\newcommand{\brimstonesecretionsdef}{%
\itemrestriction{\guidingversion{Dominant}.}
Attacks made against the model \guidingversion{and against \rnf{} models in \newrule{its unit}} no longer are \divineattacks{} (if they were).%
}
\newcommand{\thirdeyedef}{%
\newrule{At the beginning of each friendly Charge Phase, draw the Flux Card for that Player Turn instead of step 2 of the Magic Phase Sequence.}%
}
...
\itemrestriction{0--2 per Army.}
The bearer gains a \textbf{\breathattack{\St{} 3, \AP{} 2}}.%
}
...
One use only. \newrule{Must be activated w}hen the model's unit fails \newrule{its first} Charge Range roll\newrule{. The unit must} reroll the Charge Range roll.%
}
\newcommand{\piercingspikedef}{%
\itemrestriction{\guidingversion{Dominant}.}
Close Combat Attacks\removedrule{ against enemy models with an Armour value of 3 or higher} made by the model \guidingversion{and each \rnf{} model in its unit} gain\removedrule{ +1 Strength and} +1 Armour Penetration.%
}
...
\itemrestriction{\removedrule{Cannot be taken by Gigantic models. }Dominant.}
At the end of each Round of Combat during which the model's unit was Engaged in Combat, \newrule{and during which the model} \guidingversion{and \rnf{} models in the bearer's unit} \newrule{caused at least three Health Point losses with Close Combat Attacks, the unit Raises D3 Health Points}.%
}
...
\itemrestriction{\guidingversion{Dominant}.}
At the end of each Round of Combat during which the model's unit was Engaged in Combat, \newrule{and the model \guidingversion{and \rnf{} model in the bearer's unit} caused at least three Health Point losses by Close Combat Attacks,} you gain \newrule{D3} Veil Token to your Veil Token pool.%
}
\newcommand{\diviningsnoutdef}{%
\itemrestriction{\guidingversion{Dominant}.}
\newrule{When Charging a unit that contains more than one Special Item, t}he model \guidingversion{and each \rnf{} model in its unit} must reroll failed Charge Range rolls when rolling for Charge Range \newrule{and gains +\distance{2} Advance Rate for the Charge Range roll}. The effects only apply if all models in the unit are affected by \diviningsnout{}.%
}
...
\newrule{Enemy units in base contact with one or more models with this Manifestation suffer \minuss{}1 Offensive Skill and \minuss{}1 Defensive Skill.}%
}
...
\itemrestriction{\guidingversion{Dominant}.}
\newrule{The model \guidingversion{and each \rnf{} model in its unit} gains \textbf{\hatred}.}%
}
...
\itemrestriction{\guidingversion{Dominant}.}
When the model \guidingversion{or a \rnf{} model in its unit} suffers a wound from an attack with \multiplewounds{}{}, the number of wounds that it is multiplied into (due to \multiplewounds{}{}) is reduced by 1, to a minimum of 1.%
}
...
\itemrestriction{Dominant.}
Enemy units in base contact with one or more models with this Manifestation suffer \minuss{}\newrule{2} Agility.%
}
...
\itemrestriction{\guidingversion{Dominant}.}
The model \guidingversion{and each \rnf{} model in its unit} gains \textbf{\aegis{} (2+, against \flamingattacks{})}. All Melee Attacks (including Special Attacks) and Shooting Attacks made by \newrule{the} model with \incendiaryichor{} \guidingversion{and each \rnf{} model in its unit} become \textbf{\flamingattacks{}}. \newrule{The bearer \guidingversion{and each \rnf{} model in its unit} automatically fails all Fortitude Saves.}%
}
...
\itemrestriction{\guidingversion{Dominant}.}
\newrule{T}he model's Close Combat Attacks \guidingversion{and those of \rnf{} models in its unit} gain +1 Strength and +1 Armour Penetration, but each of its \newrule{Close Combat A}ttacks with a natural to-hit roll of \result{1} is distributed onto the bearer's unit.\removedrule{ The effects last until the end of the Round of Combat.}%
}
